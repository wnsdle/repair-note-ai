...
/** 텍스트 프롬프트만으로 Gemini에 JSON 응답을 요청하는 공용 함수 (음성 타이핑 정리용) */
async function callGeminiForJson(prompt: string): Promise<any | null> {
...
/**
 * 정비사가 휴대폰 키보드의 음성 타이핑(마이크) 기능으로 입력한 텍스트를
 * 정비 기록 폼에 맞는 필드로 구조화합니다.
 * 별도의 녹음/음성인식 과정 없이, 이미 텍스트로 변환된 내용을 그대로 받습니다.
 * 텍스트에 없는 정보는 빈 문자열로 둡니다(추측해서 채우지 않음).
 */
export async function parseVoiceText(text: string): Promise<VoiceTextResult | null> {
  const trimmed = text.trim();
  if (!trimmed) {
    return { symptom: "", errorCodes: "", inspection: "", cause: "" };
  }

  const prompt = `당신은 상용차(대형 화물차) 정비사가 음성 타이핑으로 입력한 메모를 정비 기록으로 정리하는 보조입니다.
아래는 정비사가 입력한 텍스트입니다. 내용을 분석해서 아래 JSON 형식으로만 답하세요.
